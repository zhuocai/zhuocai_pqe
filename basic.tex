...
\paragraph{Diffie-Hellman Problems and Gap-Diffie-Hellman groups} Let $G$ be a multiplicative group of the prime order $p$. We consider the following two problems in $G$:
\begin{definition} Computational Diffie-Hellman (CDH) problem. 
    Given $(g, u, v)$, three random elements of $G$, to compute $h=g^{\log_g u \log_g v}$. 
\end{definition}

\begin{definition} Decisional Diffie-Hellman (DDH) problem. 
    Given $(g, u, v, h)$, four elements of $G$, which with equal probability can be either all random elements of $G$ or have the property that $\log_g u = \log_v h$, to output $0$ in the former case and $1$ in the latter case. 
\end{definition}
